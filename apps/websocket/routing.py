...
def get_default_application():
    ...
    try:
        ...
        raise ImproperlyConfigured("Cannot find {!r} in ASGI_APPLICATION module {}".format(name, path))
    return value
        ...
        for route in self.routes:
            ...
            if not route.callback and isinstance(route, URLResolver):
                raise ImproperlyConfigured(
                    "%s: include() is not supported in URLRouter. Use nested" " URLRouter instances instead." % (route,)
                )
