...
from apps.websocket.db import database_sync_to_async
from apps.websocket.decorator import CookieMiddlewareDecorator, SessionMiddlewareDecorator
from apps.websocket.middleware.base import BaseMiddleware
...
def logout(scope):
    ...
    if "session" not in scope:
        raise ValueError("Login cannot find session in scope. You should wrap your " "consumer in SessionMiddleware.")
    session = scope["session"]
...
def AuthMiddlewareStack(inner):
    return CookieMiddlewareDecorator(SessionMiddlewareDecorator(AuthMiddleware(inner)))
