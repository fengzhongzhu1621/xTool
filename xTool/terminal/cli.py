...
class BaseCLIFactory:
    ...
    def get_parser(cls):
        ...
        # 遍历子解析器配置
        for subparser_name, subparser_conf in cls.subparsers_dict.items():
            # 根据子命令名创建子解析器
            sp = subparsers.add_parser(subparser_name, help=subparser_conf['help'])
            # 遍历子命令参数
            for arg in subparser_conf['args']:
                ...
                kwargs = {
                    ...
                    for f in arg_namedtuple._fields
                    if f != 'flags' and getattr(arg_namedtuple, f)
                }
                # 子解析器添加参数
                # flags：参数的短写和长写
...
def action_logging(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        assert isinstance(
            args[0], Namespace
        ), "1st positional argument should be argparse.Namespace instance, " "but {}".format(args[0])
        # 创建命令行参数的上下文
