...
def get_term_size():
    ...
    for get_terminal_size in (
        _get_term_size_posix,
        _get_term_size_windows,
        _get_term_size_environment,
        _get_term_size_tput,
    ):
        try:
...
def _get_term_size_environment():
...
def get_raw_key_function():
    ...
    """
    # Believe the first helper that doesn't bail.
    for get_raw_key_function in (_get_raw_key_function_posix, _get_raw_key_function_windows):
        try:
...
def _get_raw_key_function_posix():
...
def _get_raw_key_function_windows():
