...
class InvalidEnumValue(Error):  # pylint: disable=g-bad-exception-name
    ...
    def __init__(self, given, enum_type, options):
        ...
        """
        super().__init__(
            'Could not parse [{}] into a valid {}.  Valid values are [{}]'.format(given, enum_type, ', '.join(options))
        )
...
class OperatingSystem:
    ...
    @staticmethod
    def from_id(os_id, error_on_unknown=True):
...
class Architecture:
    ...
    @staticmethod
    def from_id(architecture_id, error_on_unknown=True):
...
"""
    ...
    def supported_version_message(self, allow_py3):
        if allow_py3:
            ...
            return 'Please use Python version {}.{}.x.'.format(
                PythonVersion.MIN_SUPPORTED_PY2_VERSION[0], PythonVersion.MIN_SUPPORTED_PY2_VERSION[1]
            )

    def is_compatible(self, allow_py3=False, raise_exception=False):
        ...
        if not self.version:
            ...
            error = 'ERROR: Your current version of Python is not compatible with ' 'the Google Cloud SDK. {}\n'.format(
                self.supported_version_message(allow_py3)
            )
        else:
            if self.version[0] < 3:
                # Python 2 Mode
                if self.version < PythonVersion.MIN_REQUIRED_PY2_VERSION:
                    error = 'ERROR: Python {}.{} is not compatible with the Google ' 'Cloud SDK. {}\n'.format(
                        self.version[0],
                        self.version[1],
                        self.supported_version_message(allow_py3),
                    )
                ...
                # Python 3 Mode
                if not allow_py3:
                    error = 'ERROR: Python 3 and later is not compatible with the ' 'Google Cloud SDK. {}\n'.format(
                        self.supported_version_message(allow_py3)
                    )
                elif self.version < PythonVersion.MIN_SUPPORTED_PY3_VERSION:
                    error = 'ERROR: Python {}.{} is not compatible with the Google ' 'Cloud SDK. {}\n'.format(
                        self.version[0], self.version[1], self.supported_version_message(allow_py3)
                    )
...
{}""".format(
                    self.supported_version_message(allow_py3), PythonVersion.ENV_VAR_MESSAGE
                )
            )
