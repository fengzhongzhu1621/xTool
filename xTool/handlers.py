...
class ContentRangeHandler:
    ...
    def __init__(self, request, stats):
        ...
        if unit != "bytes":
            raise InvalidRangeType("{} is not a valid Range Type".format(unit), self)
        start_b, _, end_b = tuple(map(str.strip, value.partition("-")))
        try:
            self.start = int(start_b) if start_b else None
        except ValueError:
            raise ContentRangeError("'{}' is invalid for Content Range".format(start_b), self)
        try:
            self.end = int(end_b) if end_b else None
        except ValueError:
            raise ContentRangeError("'{}' is invalid for Content Range".format(end_b), self)
        if self.end is None:
        ...
        self.headers = {"Content-Range": "bytes {}-{}/{}".format(self.start, self.end, self.total)}
