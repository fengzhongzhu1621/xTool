...
def set_encoded_value(env, name, value, encoding=None):
...
def encode_env(env, encoding=None):
    ...
    return {encode(k, encoding=encoding): encode(v, encoding=encoding) for k, v in env.items()}
