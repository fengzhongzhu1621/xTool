    ...
    def generate_endpoint(cls):
        for resource_route in cls.resource_routes:
            ...
            decorator_function = swagger_auto_schema(
                ...
                request_body=(
                    request_serializer if resource_route.method in ["POST", "PUT", "PATCH", "DELETE"] else None
                ),
                ...
                if resource_route.method == "GET":
                    ...
                    if resource_route.pk_field:
                        raise AssertionError(
                            gettext("当请求方法为 %s，且 endpoint 为空时，禁止设置 pk_field 参数")
                    ...
                    if not resource_route.pk_field:
                        raise AssertionError(
                            gettext("当请求方法为 %s，且 endpoint 为空时，必须提供 pk_field 参数")
                            % resource_route.method
                        )
                    setattr(cls, cls.EMPTY_ENDPOINT_METHODS[resource_route.method], function)
                else:
                    raise AssertionError(
                        gettext("不支持的请求方法: %s，请确认resource_routes配置是否正确!") % resource_route.method
                    )
            else:
