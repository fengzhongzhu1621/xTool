...
class Resource(metaclass=abc.ABCMeta):
    ...
    # 支持记录请求参数(settings开启：ENABLE_RESOURCE_DATA_COLLECT 后，
    ...
    def apply_async(self, request_data, **kwargs):
        ...
        执行celery异步任务（高级）
        """
        args = (f"{self.__module__}.{self.__class__.__name__}", get_request_username(), request_data)
        async_task = run_perform_request.apply_async(args=args, **kwargs)
