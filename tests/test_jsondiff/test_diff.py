...
class TestJsonDiffTests(unittest.TestCase):
    ...
    def test_syntax(self):
        assert diff((1, 2), (3, 4), syntax="compact") == (3, 4)
        assert diff((1, 2), (3, 4), syntax="explicit") == (3, 4)
        assert diff((1, 2), (3, 4), syntax="symmetric") == [(1, 2), (3, 4)]

        assert diff(["x", "a", "c", "x"], ["a", "b", "c"], syntax="compact") == {insert: [(1, "b")], delete: [3, 0]}
        assert diff(["x", "a", "c", "x"], ["a", "b", "c"], syntax="explicit") == {insert: [(1, "b")], delete: [3, 0]}
        assert diff(["x", "a", "c", "x"], ["a", "b", "c"], syntax="symmetric") == {
            insert: [(1, "b")],
            delete: [(3, "x"), (0, "x")],
        }
...
@pytest.mark.parametrize(
    ("a", "b", "syntax", "expected"),
    [
        pytest.param([], [{"a": True}], "explicit", {insert: [(0, {"a": True})]}, id="issue59_"),
        pytest.param([{"a": True}], [], "explicit", {delete: [0]}, id="issue59_"),
        pytest.param([], [{"a": True}], "compact", [{"a": True}], id="issue59_"),
        pytest.param([{"a": True}], [], "compact", [], id="issue59_"),
        pytest.param([], [{"a": True}], "symmetric", {insert: [(0, {"a": True})]}, id="issue59_"),
        pytest.param([{"a": True}], [], "symmetric", {delete: [(0, {"a": True})]}, id="issue59_"),
        pytest.param({1: 2}, {5: 3}, "symmetric", {delete: {1: 2}, insert: {5: 3}}, id="issue36_"),
        pytest.param({1: 2}, {5: 3}, "compact", {replace: {5: 3}}, id="issue36_"),
    ],
)
