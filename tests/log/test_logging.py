...
from xTool.log.log import LOGGING_CONFIG_DEFAULTS, logger
from xTool.response import text
from xTool.tests.testing import SanicTestClient
...
def test_logging_defaults():
    ...
    for fmt in [h.formatter for h in logging.getLogger("xTool.root").handlers]:
        assert (
            fmt._fmt
    ...
    for fmt in [
        h.formatter for h in logging.getLogger("xTool.error").handlers
    ]:
        assert (
            fmt._fmt
            == LOGGING_CONFIG_DEFAULTS["formatters"]["generic"]["format"]
    ...
    for fmt in [
        h.formatter for h in logging.getLogger("xTool.access").handlers
    ]:
        assert (
            fmt._fmt
...
def test_log_connection_lost(app, debug, monkeypatch):
    ...
    stream = StringIO()
    root = logging.getLogger("xTool.root")
    root.addHandler(logging.StreamHandler(stream))
    monkeypatch.setattr(server, "logger", root)
...
def test_logger(caplog):
    ...
    assert caplog.record_tuples[1] == ("xTool.root", logging.INFO, rand_string)
...
def test_logger_static_and_secure(caplog):
    ...
    assert caplog.record_tuples[2] == ("xTool.root", logging.INFO, rand_string)
    assert caplog.record_tuples[-1] == (
        "xTool.root",
        logging.INFO,
        "Server Stopped",
...
def test_logging_modified_root_logger_config():
    ...
    modified_config = LOGGING_CONFIG_DEFAULTS
    modified_config["loggers"]["xTool.root"]["level"] = "DEBUG"

    app = Sanic("test_logging", log_config=modified_config)

    assert logging.getLogger("xTool.root").getEffectiveLevel() == logging.DEBUG
