class CacheTestMixin:
    ...
    def test_defaults(self):
        ...
        self.assertEqual(1, cache.get_sizeof(0))
    ...
    def test_insert(self):
        ...
        cache[3] = 3  # 调用__setitem__，缓存区满，替换最后一个
        self.assertEqual(2, len(cache))
        self.assertEqual(3, cache[3])
        self.assertTrue(1 in cache or 2 in cache)

        cache[4] = 4  # 调用__setitem__，缓存区满，替换最后一个
        self.assertEqual(2, len(cache))
    ...
    def test_update(self):
        ...
        cache.update({1: 1, 2: 2})  # 使用相同的值覆盖缓存区的数据
        self.assertEqual(2, len(cache))
        self.assertEqual(1, cache[1])
        self.assertEqual(2, cache[2])

        cache.update({1: "a", 2: "b"})  # 使用不相同的值覆盖缓存区的数据
        self.assertEqual(2, len(cache))
    ...
    def test_missing_get_sizeof(self):
        class DefaultCache(self.Cache):
            def __missing__(self, key):
                try:
                    self[key] = key
        ...
        self.assertEqual(1, cache[1])
        self.assertEqual(1, len(cache))
        self.assertEqual(1, cache.curr_size)
        self.assertIn(1, cache)

        self.assertEqual(2, cache[2])
        self.assertEqual(1, len(cache))
        self.assertEqual(2, cache.curr_size)
        self.assertNotIn(1, cache)
    ...
    def _test_get_sizeof(self, cache):
        ...
        cache.update({1: 1, 2: 2})
        self.assertEqual(2, len(cache))
        self.assertEqual(3, cache.curr_size)
        self.assertEqual(1, cache[1])
        self.assertEqual(2, cache[2])
